...
W5500Class w5500;
PioSPI spiBus(23, 22, 21, 20, SPI_MODE0, 20000000); //MOSI, MISO, SCK, CS, CPHA, CPOL, FREQUENCY
...
{
    spiBus.beginTransaction(wiznet_SPI_settings);
    setSS();
    spiBus.transfer(_addr >> 8);
    spiBus.transfer(_addr & 0xFF);
    spiBus.transfer(_cb);
    spiBus.transfer(_data);
    resetSS();
...
uint8_t W5500Class::read(uint16_t _addr, uint8_t _cb)
{
    ...
    spiBus.transfer(_addr & 0xFF);
    spiBus.transfer(_cb);
    uint8_t _data = spiBus.transfer(0);
    resetSS();
    spiBus.endTransaction();
...
uint8_t W5500Class::readVersion(void)
{
    ...
    spiBus.transfer( 0x39 );
    spiBus.transfer( 0x01);
    uint8_t _data = spiBus.transfer(0);
    resetSS();
    spiBus.endTransaction();
